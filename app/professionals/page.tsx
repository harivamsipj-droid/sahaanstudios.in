...
export default function ProfessionalsPage() {
  ...
  return <main className="directory-page hyderabad-directory">
    ...
    <section className="directory-hero directory-working-hero"><a href="/" className="back-link"><ArrowLeft /> Back to customer website</a><p className="eyebrow">Hyderabad-wide discovery and enquiries</p><h1>Search one PIN code.<br /><em>Compare nearby listings.</em></h1><p>Explore Google-listed nail and beauty businesses around Hyderabad, then ask Sahaan about a managed home-service match. Listings are not bookable or Sahaan-verified unless separately confirmed.</p>
      ...
      <div className="manual-match-panel"><div><strong>Would you like Sahaan to find an artist?</strong><p>Send a request from any Hyderabad neighbourhood. We’ll check whether a reviewed artist serves your area and confirm availability, treatment details and the full quote before any booking.</p><small>A WhatsApp request is not a confirmed appointment. Do not pay until Sahaan shares the final terms.</small></div><a href={sahaanWhatsAppUrl(matchMessage)} target="_blank" rel="noopener noreferrer"><MessageCircle /> Request a Sahaan match</a></div>
